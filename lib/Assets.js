...
class Assets {
  ...
  filterAssets(assets) {
    return assets.filter(
      (asset) =>
        Object.keys(asset.fields).length &&
        asset.fields.file &&
        asset.fields.file &&
  ...
  async upsertAndDelete() {
    const { updatedAssets, newAssets, removedAssets, environment } = this;
    if (updatedAssets.length) {
      console.log('Updating assets...');
      console.log(JSON.stringify(updatedAssets));
      await this.updateAssets(updatedAssets, environment);
    ...
    if (newAssets.length) {
      ...
      await this.createAssets(newAssets, environment);
    ...
    if (removedAssets.length) {
      ...
      await this.removeAssets(removedAssets, environment);
      console.log('Successfully removed assets');
    } else {
      console.log('No assets to remove');
    }
  }
  async updateAssets(updatedAssets, environment) {
    return await asyncForEach(updatedAssets, async (asset) => {
      try {
        const foundAsset = await environment.getAsset(asset.sys.id);
        delete asset.sys;
  ...
  async createAssets(newAssets, environment) {
    return await asyncForEach(newAssets, async (asset) => {
      try {
        const createdAsset = await environment.createAssetWithId(
          asset.sys.id,
          asset
        );
  ...
  async removeAssets(removedAssets, environment) {
